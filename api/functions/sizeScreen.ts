...
export const sizeScreen = (): void => {
  ...
  if (screenElement === null) {
    throw new Error("An attempt was made to size screen with no screen element in the DOM.");
  }
  ...
  if (pauseMenuElement === null) {
    throw new Error("An attempt was made to size screen with no pause menu element in the DOM.");
  }
  pauseMenuElement.style.width = `${screenWidth}px`;
  pauseMenuElement.style.height = `${screenHeight}px`;
  const lowerDimension: number = Math.min(state.values.config.width, state.values.config.height);
  ...
  for (const element of document.getElementsByClassName("pause-toggle")) {
    if (!(element instanceof HTMLElement)) {
      throw new Error("An element with class pause-toggle was not an HTMLElement.");
    }
